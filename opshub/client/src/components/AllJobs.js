...
export default function AllJobs({ jobs = [], onRefresh, title }) {
  ...
  async function createJob() {
    ...
    try {
      ...
      setShowNew(false);
      setNewJob({title:'',address:'',status:'scheduled',scheduledDate:''});
      onRefresh();
    } catch(err) {
  ...
  return (
    <div>
      <div style={{display:'grid',gridTemplateColumns:'repeat(4,1fr)',gap:10,marginBottom:18}}>
        {[
          ['Total', jobs.length, 'var(--tx)'],
          ['In progress', jobs.filter(j=>j.status==='in-progress').length, 'var(--blue2)'],
          ['On hold', jobs.filter(j=>j.status==='on-hold').length, 'var(--amber2)'],
          ['Scheduled', jobs.filter(j=>j.status==='scheduled').length, 'var(--green2)']
        ].map(([l,v,c]) => (
      ...
      <div style={{display:'flex',gap:5,marginBottom:14,flexWrap:'wrap',alignItems:'center'}}>
        {['all','in-progress','on-hold','scheduled','completed'].map(s => (
          <button key={s}
            style={{padding:'3px 10px',borderRadius:20,border:`1px solid ${filter===s?'rgba(58,123,213,.5)':'rgba(255,255,255,.08)'}`,background:filter===s?'rgba(58,123,213,.2)':'transparent',fontSize:11,cursor:'pointer',color:filter===s?'var(--blue2)':'var(--tx3)',fontFamily:"'DM Sans',sans-serif",transition:'all .15s'}}
            onClick={() => setFilter(s)}>
            {s==='all'?'All':s.replace('-',' ')}
          </button>
        ))}
      ...
      <div style={{display:'grid',gridTemplateColumns:'repeat(3,1fr)',gap:11}}>
        {filtered.map(j => {
          ...
          return (
            <div key={j.id}
              style={{background:'var(--bg2)',border:'1px solid var(--border)',borderRadius:'var(--r2)',overflow:'hidden',cursor:'pointer',transition:'all .18s'}}
              ...
              <div style={{padding:'9px 13px'}}>
                <div style={{display:'flex',justifyContent:'space-between',fontSize:10,color:'var(--tx3)',marginBottom:3}}>
                  <span>{j.tasks.filter(t=>t.status==='done').length}/{j.tasks.length} tasks</span>
                  <span>{p}%</span>
                </div>
                ...
                <div style={{display:'flex',gap:3,flexWrap:'wrap',marginTop:7}}>
                  {taskTypes.map(n => {
                    const tt = TASK_TYPES.find(t=>t.id===n);
                    return tt ? <span key={n} className="tag" style={{background:tt.color+'18',color:tt.color,fontSize:10}}>{n}</span> : null;
                  })}
                </div>
              </div>
      ...
      {showNew && (
        <div className="overlay" onClick={e => { if(e.target===e.currentTarget) setShowNew(false); }}>
          <div className="modal" style={{maxWidth:490}} onClick={e=>e.stopPropagation()}>
            <div className="mhd">
              <div style={{fontSize:14,fontWeight:600,flex:1}}>New job</div>
              <button className="btn btn-sm" style={{padding:'4px 7px'}} onClick={()=>setShowNew(false)}><i className="fa fa-times"></i></button>
            </div>
            <div className="mbody">
              ...
              <div className="fgrid">
                <div className="fg"><label className="fl">Status</label>
                  <select className="fs" value={newJob.status} onChange={e=>setNewJob({...newJob,status:e.target.value})}>
            ...
            <div className="mfoot">
              ...
              <button className="btn btn-blue" onClick={createJob}><i className="fa fa-check"></i> Create</button>
            </div>
          </div>
        </div>
      )}
      ...
      {selectedJob && (
        <JobDetailModal
          ...
          onRefresh={() => { onRefresh(); setSelectedJob(null); }}
        />
      )}
    </div>
  );
...
function JobDetailModal({ job, onClose, onRefresh }) {
  ...
  const [newTask, setNewTask] = useState({ name: 'Siding', crewId: '', startDate: '', duration: 5 });

  useEffect(() => { loadJob(); }, [job.id]);
  ...
  async function addTask() {
    ...
    try {
      await api.post(`/jobs/${job.id}/tasks`, newTask);
      const res = await api.get(`/jobs/${job.id}`);
      setFullJob(res.data);
      setNewTask({ name: 'Siding', crewId: '', startDate: '', duration: 5 });
    } catch(err) {
  ...
  async function deleteTask(taskId) {
    ...
    try {
      await api.delete(`/jobs/${job.id}/tasks/${taskId}`);
      const res = await api.get(`/jobs/${job.id}`);
      setFullJob(res.data);
    } catch(err) {
  ...
  function crewsForTrade(tradeName) {
    return crews.filter(c => (c.trades || []).includes(tradeName));
  }
      ...
      <div className="modal" onClick={e=>e.stopPropagation()}>
        <div className="mhd">
          <div style={{flex:1}}>
            ...
            <div style={{fontSize:11,color:'var(--tx3)'}}><i className="fa fa-map-marker-alt"></i> {job.address}</div>
          </div>
          <button className="btn btn-sm" style={{padding:'4px 7px'}} onClick={onClose}><i className="fa fa-times"></i></button>
        </div>
        <div className="mtabs">
          {tabs.map(t => <div key={t.id} className={`mtab${tab===t.id?' on':''}`} onClick={() => setTab(t.id)}>{t.l}</div>)}
        </div>
        <div className="mbody">
          {loading ? (
            <div style={{textAlign:'center',padding:24}}><div className="spin" style={{margin:'0 auto'}}></div></div>
          ) : (
            <>
              {tab === 'tasks' && (
                <div>
                  {fullJob?.tasks?.length > 0 && (
                    <table style={{width:'100%',borderCollapse:'collapse',border:'1px solid var(--border)',borderRadius:'var(--r)',overflow:'hidden',marginBottom:14}}>
                      <thead>
                        <tr style={{background:'var(--bg3)'}}>
                          ...
                          <th style={{padding:'7px 10px',fontSize:10,fontWeight:600,color:'var(--tx3)',textAlign:'left',textTransform:'uppercase',letterSpacing:'.06em'}}>Start date</th>
                          ...
                          {can('editTasks') && <th style={{width:32}}></th>}
                        </tr>
                      </thead>
                      <tbody>
                        {fullJob.tasks.map(t => {
                          const tt = TASK_TYPES_MAP[t.name];
                          const specialists = crewsForTrade(t.name);
                          const behind = t.status==='pending' && t.start_date && new Date(t.start_date) < new Date();
                          return (
                            <tr key={t.id} style={{borderTop:'1px solid var(--border)',background:behind?'rgba(231,76,60,.05)':'transparent'}}>
                              <td style={{padding:'8px 10px'}}>
                                <div style={{display:'flex',alignItems:'center',gap:6}}>
                                  {tt && <div style={{width:16,height:16,borderRadius:4,background:tt.color,display:'flex',alignItems:'center',justifyContent:'center'}}><i className={`fa ${tt.icon}`} style={{fontSize:9,color:'#fff'}}></i></div>}
                                  <span style={{fontWeight:500,fontSize:12}}>{t.name}</span>
                                  {behind && <span className="tag tag-red" style={{fontSize:9}}>overdue</span>}
                                </div>
                              </td>
                              <td style={{padding:'8px 10px',fontSize:12}}>
                                {can('editTasks') ? (
                                  <select className="fs" style={{padding:'2px 5px',fontSize:11,minWidth:130}}
                                    value={t.crew_id||''}
                                    onChange={e => updateTask(t.id, {crewId:e.target.value||null,status:t.status,startDate:t.start_date,duration:t.duration})}>
                                    <option value="">— Assign —</option>
                                    {specialists.map(c => <option key={c.id} value={c.id}>{c.name}</option>)}
                                    {specialists.length===0 && <option disabled>No specialists yet</option>}
                                  </select>
                                ) : (t.crew_name||'Unassigned')}
                              </td>
                              <td style={{padding:'8px 10px',fontSize:12}}>
                                {can('editTasks') ? (
                                  <input type="date" className="fi" style={{padding:'2px 5px',fontSize:11,minWidth:110}}
                                    value={t.start_date||''}
                                    onChange={e => updateTask(t.id, {crewId:t.crew_id,status:t.status,startDate:e.target.value,duration:t.duration})}/>
                                ) : (t.start_date||'—')}
                              </td>
                              <td style={{padding:'8px 10px',fontSize:12}}>
                                {can('editTasks') ? (
                                  <select className="fs" style={{padding:'2px 5px',fontSize:11}}
                                    value={t.status}
                                    onChange={e => updateTask(t.id, {crewId:t.crew_id,status:e.target.value,startDate:t.start_date,duration:t.duration})}>
                              ...
                              {can('editTasks') && (
                                <td style={{padding:'8px 6px'}}>
                                  <button className="btn btn-sm" style={{padding:'2px 5px',color:'var(--red2)'}} onClick={() => deleteTask(t.id)}>
                  ...
                  {can('editTasks') && (fullJob?.tasks?.length||0) < 10 && (
                    <div style={{background:'var(--bg3)',borderRadius:'var(--r)',padding:'12px 14px',border:'1px solid var(--border)'}}>
                      <div style={{fontSize:11,fontWeight:600,color:'var(--tx2)',marginBottom:10,textTransform:'uppercase',letterSpacing:'.07em'}}>Add task</div>
                      <div style={{display:'grid',gridTemplateColumns:'1fr 1fr 1fr auto',gap:8,alignItems:'end'}}>
                        <div>
                          <label className="fl">Trade type</label>
                          <select className="fs"
                            value={newTask.name}
                            onChange={e => setNewTask({...newTask, name:e.target.value, crewId:'', duration:DD[e.target.value]||1})}>
                        ...
                        <div>
                          <label className="fl">
                            Crew
                            {crewsForTrade(newTask.name).length > 0
                              ? ` (${crewsForTrade(newTask.name).length} specialist${crewsForTrade(newTask.name).length!==1?'s':''})`
                              : ' — no specialists'}
                          </label>
                          <select className="fs"
                            value={newTask.crewId}
                            onChange={e => setNewTask({...newTask, crewId:e.target.value})}>
                            <option value="">Assign later…</option>
                            {crewsForTrade(newTask.name).map(c => <option key={c.id} value={c.id}>{c.name}</option>)}
                          </select>
                        </div>
                        <div>
                        ...
                        <button className="btn btn-blue" style={{height:34,marginTop:'auto'}} onClick={addTask}>
                  ...
                  {!fullJob?.tasks?.length && !can('editTasks') && (
                    <div className="empty"><i className="fa fa-tasks"></i>No tasks yet</div>
                  )}
                </div>
              )}
              ...
              {tab === 'pos' && (
                <div>
                  <div style={{fontSize:11,color:'var(--tx3)',marginBottom:10,display:'flex',alignItems:'center',gap:5}}>
                    <i className="fa fa-lock" style={{color:'var(--amber2)'}}></i> Visible to Owner, Admin, Driver only
                  </div>
                  {(fullJob?.pos||[]).length===0 && <div style={{fontSize:12,color:'var(--tx3)',padding:'7px 0'}}>No POs yet</div>}
                  {(fullJob?.pos||[]).map(p => (
                  ...
                  {can('editJobs') && <FileUploader jobId={job.id} type="po" onUploaded={loadJob}/>}
                </div>
              )}
              ...
              {tab === 'colours' && (
                <div>
                  <div style={{fontSize:11,color:'var(--tx3)',marginBottom:10,display:'flex',alignItems:'center',gap:5}}>
                    <i className="fa fa-lock" style={{color:'var(--paint2)'}}></i> Visible to Owner, Admin, Painter, Driver only
                  </div>
                  ...
                  {(fullJob?.colours||[]).length===0 && <div style={{fontSize:12,color:'var(--tx3)'}}>No colours yet</div>}
                </div>
              )}
                  ...
                  {(fullJob?.blueprints||[]).map(b => (
                    <div key={b.id} style={{background:'var(--bg3)',border:'1px solid var(--border)',borderRadius:'var(--r)',padding:'10px 13px',display:'flex',alignItems:'center',gap:9,marginBottom:7}}>
                      ...
                      <div style={{flex:1}}>
                        ...
                        <div style={{fontSize:10,color:'var(--tx3)',textTransform:'capitalize'}}>{b.category}</div>
                      </div>
                      {b.file_url && <a href={b.file_url} target="_blank" rel="noreferrer" className="btn btn-blue btn-sm"><i className="fa fa-eye"></i> View</a>}
                    </div>
                  ))}
                  {can('editJobs') && <FileUploader jobId={job.id} type="blueprint" onUploaded={loadJob}/>}
                </div>
              )}
              ...
              {tab === 'delivery' && (
                <div>
                  <div style={{fontSize:12,color:'var(--tx2)',marginBottom:12}}>
                    Delivery status: <span style={{fontWeight:600}}>{fullJob?.delivery_status||'pending'}</span>
                  </div>
        ...
        <div className="mfoot">
          {can('editJobs') && (
            <button className="btn btn-sm" style={{color:'var(--red2)',marginRight:'auto'}}
              onClick={async () => {
                if (!window.confirm('Delete this job?')) return;
                await api.delete(`/jobs/${job.id}`);
                onRefresh();
              }}>
              <i className="fa fa-trash"></i> Delete job
            </button>
          )}
...
function FileUploader({ jobId, type, onUploaded }) {
  ...
  return (
    <div style={{border:'2px dashed var(--border2)',borderRadius:'var(--r2)',padding:'18px 20px',textAlign:'center',marginTop:9,position:'relative',background:'var(--sf)'}}>
      ...
      {uploading ? (
        ...
        <>
          ...
          <div style={{fontSize:12,color:'var(--tx2)',fontWeight:500}}>Upload {type==='po'?'PO':'blueprint or colour selection'}</div>
          <div style={{fontSize:11,color:'var(--tx3)',marginTop:3}}>PDF, JPG, or PNG · Max 50MB</div>
        </>
      )}
    </div>
  );
